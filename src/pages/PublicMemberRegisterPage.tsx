...
import { Users, Lock, Loader2, CheckCircle2, ShieldCheck, Eye, EyeOff, Mail, Phone } from 'lucide-react';
import { SmokeyBackground } from '../components/ui/SmokeyBackground';

/**
 * Cadastro de membro de equipe (Coordenador/Líder/Apoiador) via link de convite
 * em cadeia (#149). O nome e o papel vêm travados do convite — o convidado só
 * define e-mail e senha. Entra na MESMA campanha do candidato.
 */
interface Invite { displayName: string; role: string; candidateName: string | null; phone?: string | null; alreadyRegistered: boolean; }
...
const ROLE_LABEL: Record<string, string> = {
  'Coordenador': 'Coordenador', 'Líder': 'Líder', 'Lider': 'Líder', 'Apoiador': 'Equipe de apoio',
};

const PublicMemberRegisterPage: React.FC = () => {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        const r = await fetch(`/api/public/party/member-invite/${token}`);
        const j = await r.json();
        if (!r.ok) throw new Error(j.error || 'Convite inválido');
        setInvite(j);
        if (j.phone) setForm((f) => ({ ...f, phone: j.phone }));
  ...
  const submit = async () => {
    ...
    try {
      const r = await fetch(`/api/public/party/member-invite/${token}/register`, {
      ...
      <div className="relative z-10 flex min-h-screen flex-col items-center justify-center p-4">
        <div className="w-full max-w-sm space-y-5 rounded-2xl border border-white/15 bg-white/5 p-8 shadow-2xl backdrop-blur-xl">
          <div className="text-center">
            ...
            {invite?.candidateName && <p className="text-[11px] uppercase tracking-widest text-indigo-400 font-bold">Equipe de {invite.candidateName}</p>}
            <h1 className="text-2xl font-black text-white mt-1">Cadastro de {roleLabel}</h1>
          </div>

          {/* Nome + papel travados (vínculo garantido pelo convite) */}
          ...
          <p className="text-[11px] text-slate-500 text-center flex items-center justify-center gap-1.5"><ShieldCheck className="w-3.5 h-3.5" /> Você entra na equipe da campanha — seus dados ficam restritos a ela.</p>
        </div>
      </div>
    </main>
  );
...
export default PublicMemberRegisterPage;
