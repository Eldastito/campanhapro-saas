...
import { Headset, Lock, Loader2, CheckCircle2, ShieldCheck, Eye, EyeOff, Mail, Phone } from 'lucide-react';
import { SmokeyBackground } from '../components/ui/SmokeyBackground';

/**
 * Cadastro de líder/operador de call center via link de convite (público).
 * O nome vem do convite e fica TRAVADO — garante o vínculo com a campanha.
 * O convidado só define e-mail e senha.
 */
interface Invite { displayName: string; role: string; campaignName: string; alreadyUsed: boolean; }
...
const PublicCallCenterRegisterPage: React.FC = () => {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        const r = await fetch(`/api/public/callcenter/invite/${token}`);
  ...
  const submit = async () => {
    ...
    try {
      const r = await fetch(`/api/public/callcenter/register/${token}`, {
      ...
      <main className="relative min-h-screen w-full overflow-hidden bg-slate-950">
        ...
        <div className="relative z-10 flex min-h-screen items-center justify-center p-6">
          <div className="max-w-md w-full rounded-2xl border border-white/15 bg-white/5 p-8 shadow-2xl backdrop-blur-xl text-center">
            ...
            <p className="text-sm text-slate-300 mt-1 mb-5">Sua conta de atendimento está pronta. Faça login para começar.</p>
      ...
      <div className="relative z-10 flex min-h-screen flex-col items-center justify-center p-4">
        <div className="w-full max-w-sm space-y-5 rounded-2xl border border-white/15 bg-white/5 p-8 shadow-2xl backdrop-blur-xl">
          <div className="text-center">
            <Headset className="w-10 h-10 text-indigo-400 mx-auto mb-2" />
            <p className="text-[11px] uppercase tracking-widest text-indigo-400 font-bold">{invite?.campaignName}</p>
            <h1 className="text-2xl font-black text-white mt-1">Equipe de Atendimento</h1>
          </div>

          {/* Nome + função travados (vínculo garantido pelo convite) */}
          <div className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 flex items-center gap-2">
            ...
            <div>
              ...
              <p className="font-bold text-white">{invite?.displayName}</p>
              <p className="text-xs text-indigo-300">{invite?.role}</p>
            </div>
          </div>
          ...
          <p className="text-[11px] text-slate-500 text-center flex items-center justify-center gap-1.5"><ShieldCheck className="w-3.5 h-3.5" /> Acesso restrito ao atendimento — você verá apenas as conversas da fila.</p>
        </div>
      </div>
    </main>
  );
...
export default PublicCallCenterRegisterPage;
