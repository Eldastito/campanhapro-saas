...
import { Trophy, Maximize2, Vote, MapPin, Target, Gauge, Settings2, Radio } from 'lucide-react';
 ...
 */
interface BURow { votosCandidato: number; votosTotalSecao: number; zona: string | null; secao: string | null; }
  ...
  React.useEffect(() => {
    ...
    const fetchRows = async () => {
      const { data } = await supabase.from('boletins_urna')
        .select('votosCandidato, votosTotalSecao, zona, secao').eq('campaignId', user.campaignId);
