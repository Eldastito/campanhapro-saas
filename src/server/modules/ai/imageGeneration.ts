// Image generation helper. Tries Google Gemini first (codename "nano-banana");
// falls back to OpenAI gpt-image-1 if Gemini is unavailable OR errors out.
...
async function generateViaGemini(prompt: string): Promise<{ dataUrl: string; provider: 'gemini' }> {
  const model = process.env.AI_MODEL_IMAGE_GEMINI || 'gemini-2.0-flash-preview-image-generation';
...
async function generateViaOpenAI(prompt: string, size: GenerateArgs['size']): Promise<{ dataUrl: string; provider: 'openai' }> {
...
export async function generateImage(args: GenerateArgs): Promise<{ dataUrl: string; provider: 'gemini' | 'openai' }> {
  ...
  if (hasGemini) {
    try {
      ...
      console.warn('[Image] Gemini falhou, tentando fallback OpenAI:', err.message);
      if (!hasOpenAI) throw err;
    }
  }

  if (hasOpenAI) {
    return await generateViaOpenAI(prompt, size);
  }
